...
Local `max_parallel > 1` requires explicit disjoint GPU groups under `[scheduler.gpu_assignment]`. SLURM sweeps submit jobs and exit, so early stopping and Optuna are rejected with `scheduler.type = "slurm"`.
